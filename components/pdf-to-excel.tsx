...
import { PdfToExcelAutomaticChecks } from "@/components/pdf-to-excel-automatic-checks";
...
import { tableViewMergeNotice, tableViewPlan, tableViewSelection } from "@/lib/pdf/table-view";
import {
...
import { verifyPdfTable } from "@/lib/pdf/table-verification";
...
export function PdfToExcel() {
  ...
  const verification = useMemo(() => tables[selected] && result?.tables[selected]
    ? verifyPdfTable(result.tables[selected], tables[selected])
    : null, [result, selected, tables]);
  ...
  // Display-only merge rendering: table.merges turns into real colSpan/rowSpan and covered
  // coordinates are not rendered at all. The matrix, its values and all provenance stay untouched.
  const tableView = useMemo(() => tableViewPlan(table), [table]);
  // An externally selected coordinate inside a merged region highlights its real anchor cell.
  const visibleSelection = tableViewSelection(tableView, activeCell);
  const mergeNotice = tableViewMergeNotice(tableView);
  ...
  return <section className="pdf-split-shell pdf-to-excel-shell" aria-labelledby="pdf-to-excel-title">
    ...
    {result && <div className="pdf-excel-result" role="status">
      {tables.length === 0 ? <Card className="pdf-excel-empty"><strong>No clear tables found</strong><p>The PDF was read successfully, but no regular table structure could be identified confidently. Try a PDF with clearer rows and columns.</p></Card> : <>
        ...
        {table && <Card className="pdf-excel-preview"><div className="pdf-excel-preview-head"><div><strong>{table.name}</strong><span>{table.rows.length} rows · {Math.max(0, ...table.rows.map((row) => row.length))} columns · {table.source === "ocr" ? "OCR" : "PDF text"}</span></div>{csvUrl && <a className={buttonClassName({ variant: "secondary" })} href={csvUrl} download={makeTableCsvFilename(source?.file.name ?? "document.pdf", table.name)}>Download this table (.csv)</a>}</div><p className="pdf-excel-edit-note">Review and edit cells before downloading. Changes are kept only in this browser session.</p>{verification && <PdfToExcelAutomaticChecks verification={verification} onReviewCell={setActiveCell} />}<div className="pdf-excel-review-head"><h3 className="pdf-excel-review-title" id="pdf-to-excel-source-review-title">{SOURCE_REVIEW_COPY.heading}</h3><p className="pdf-excel-review-instruction">{SOURCE_REVIEW_COPY.instruction}</p></div><div className="pdf-excel-review"><section className="pdf-excel-review-source" aria-labelledby="pdf-to-excel-source-review-title">{source && <PdfToExcelSourceReview file={source.file} totalPages={result?.pageCount ?? source.pageCount} evidence={selection.evidence} previewEvidence={selection.previewEvidence} retainPageContext={selection.retainPageContext} notice={selection.notice} cell={selection.cell} mergedAnchor={selection.mergedAnchor} />}</section><section className="pdf-excel-review-table"><div className="pdf-excel-source-toolbar pdf-excel-table-toolbar"><span className="pdf-excel-source-toolbar-label">{SOURCE_REVIEW_COPY.tableLabel}</span><div className="pdf-excel-source-zoom"><Button variant="ghost" size="sm" aria-label="Zoom out table" disabled={tableZoom <= TABLE_VIEW_MIN_ZOOM} onClick={() => setTableZoom(clampTableViewZoom(tableZoom - TABLE_VIEW_ZOOM_STEP))}>{"−"}</Button><span className="pdf-excel-source-zoom-value">{`${Math.round(tableZoom * 100)}%`}</span><Button variant="ghost" size="sm" aria-label="Zoom in table" disabled={tableZoom >= TABLE_VIEW_MAX_ZOOM} onClick={() => setTableZoom(clampTableViewZoom(tableZoom + TABLE_VIEW_ZOOM_STEP))}>+</Button></div></div>{table.title ? <p className="pdf-excel-table-title">{table.title}</p> : null}<div className="pdf-excel-table-scroll" style={{ "--table-zoom": `${tableZoom}` } as CSSProperties}><table><tbody>{tableView.rows.map((row, rowIndex) => <tr key={rowIndex}>{row.map((cell) => <td key={`${cell.row}:${cell.column}`} rowSpan={cell.rowSpan > 1 ? cell.rowSpan : undefined} colSpan={cell.colSpan > 1 ? cell.colSpan : undefined} className={visibleSelection && visibleSelection.row === cell.row && visibleSelection.column === cell.column ? "is-source-selected" : undefined}><AutoGrowTextarea aria-label={`${table.name}, row ${cell.row + 1}, column ${cell.column + 1}`} zoom={tableZoom} value={String(table.rows[cell.row][cell.column] ?? "")} onFocus={() => setActiveCell({ row: cell.row, column: cell.column })} onClick={() => setActiveCell({ row: cell.row, column: cell.column })} onChange={(event) => editCell(cell.row, cell.column, event.target.value)} /></td>)}</tr>)}</tbody></table></div>{mergeNotice && <p className="pdf-excel-edit-note">{mergeNotice}</p>}</section></div></Card>}
      </>}
    </div>}
