...
const {recognitionLines}=require('../lib/pdf/ocr-word-layout.ts');
...
async function recognizePage(worker,page,candidates=[0]){
  ...
  try{
    ...
    return searchable.searchableOcrWordPlacements(recognitionLines(best.data),best.image.width,best.image.height,best.rotation,viewport);
...
async function makeOutput(pdfjs,worker,bytes,rotationCandidates={}){
  const doc=await openPdf(pdfjs,bytes),layers=new Map(),preserved=[],sanitize=new Set();
  try{for(let number=1;number<=doc.numPages;number++){const page=await doc.getPage(number);try{const text=await page.getTextContent();if(searchable.hasUsablePdfText(text)){preserved.push(number);continue;}const viewport=page.getViewport({scale:1}),existing=searchable.sourceTextPlacements(text,await page.getOperatorList(),pdfjs.OPS),plan=searchable.planSearchableTextLayer(await recognizePage(worker,page,rotationCandidates[number]||[0]),existing,viewport.width,viewport.height);if(plan.sanitizeInvisibleSourceText)sanitize.add(number);layers.set(number,plan.placements);}finally{page.cleanup();}}}finally{await doc.loadingTask.destroy();}
  return {bytes:layers.size?await searchable.addSearchableTextLayers(new Uint8Array(bytes),layers,sanitize):new Uint8Array(bytes),layers,preserved,sanitize};
}
async function pageText(pdfjs,bytes){const doc=await openPdf(pdfjs,bytes),texts=[];try{for(let i=1;i<=doc.numPages;i++){const page=await doc.getPage(i);try{texts.push((await page.getTextContent()).items.map(item=>item.str).join(' '));}finally{page.cleanup();}}return texts;}finally{await doc.loadingTask.destroy();}}
async function searchBoxes(pdfjs,bytes,term){const doc=await openPdf(pdfjs,bytes),matches=[];try{for(let i=1;i<=doc.numPages;i++){const page=await doc.getPage(i);try{for(const item of (await page.getTextContent()).items)if(new RegExp(term,'i').test(item.str))matches.push({page:i,text:item.str,x:item.transform[4],y:item.transform[5],width:item.width,height:item.height});}finally{page.cleanup();}}return matches;}finally{await doc.loadingTask.destroy();}}
...
(async()=>{
  ...
  try{
    ...
    const sparseSourceText=await pageText(pdfjs,sources.sparse),sparseOutputText=await pageText(pdfjs,sparse.bytes),occurrences=(values,term)=>(values.join(' ').match(new RegExp(term,'gi'))||[]).length;assert.equal(occurrences(sparseSourceText,'CamScanner'),4);assert.equal(occurrences(sparseOutputText,'CamScanner'),4);assert.equal(occurrences(sparseSourceText,'Punjab'),4);assert.equal(occurrences(sparseOutputText,'Punjab'),4);assert.equal(sparse.layers.size,4);assert.equal(sparse.sanitize.size,4);sparseOutputText.forEach((text,index)=>assert.match(normalizedText(text),new RegExp(`Punjab searchable sentence ${index+1}`,'i')));
    const punjabBoxes=await searchBoxes(pdfjs,sparse.bytes,'Punjab');assert.equal(punjabBoxes.length,4);punjabBoxes.forEach(box=>{assert.ok(box.x<80,`Punjab highlight remained in blank area at x=${box.x}`);assert.ok(box.y>260&&box.y<320,`Punjab highlight baseline was misplaced at y=${box.y}`);});
    ...
    console.log(JSON.stringify({output,scannedText,mixedText,rotatedText,sparseOccurrences:{camScanner:occurrences(sparseOutputText,'CamScanner'),punjab:occurrences(sparseOutputText,'Punjab')},punjabHighlightBoxes:punjabBoxes,meanPixelDifference:meanPixelDifference(sourcePixels,outputPixels)}));
